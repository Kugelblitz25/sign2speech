...
def load_and_crop_video(video_path, bbox, resize_dim=(224, 224)):
    try:
        ...
        for i in range(video_frames.shape[1]):
            ...
            frame_resized = torch.nn.functional.interpolate(frame.unsqueeze(0), size=resize_dim).squeeze(0)
...
def save_concatenated_video(frames_list, output_path, fps=30):
    ...
    for frames in frames_list:
        for frame_tensor in frames:
            frame_np = (255 - frame_tensor * 255).byte().permute(1, 2, 0).numpy().astype('uint8')
...
def create_combined_videos(data: list[dict], video_root: str, output_dir: str, num_videos: int = 5):
    ...
    for i in tqdm(range(num_videos), desc="Creating combined videos"):
        ...
        if len(frames_list) == 3:
            combined_video_path = output_dir / f"{'_'.join([item['gloss'] for item in selected_items])}.mp4"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--datafile', type=str, default='data/raw/WLASL_v0.3.json', help='Path to JSON file containing video information')
    ...
    parser.add_argument('--output_dir', type=str, default='test_videos', help='Directory to save combined videos')
    parser.add_argument('--num_videos', type=int, default=1, help='Number of combined videos to create (default: 5)')
    ...
    create_combined_videos(data, Path(args.video_root), args.output_dir, args.num_videos)
