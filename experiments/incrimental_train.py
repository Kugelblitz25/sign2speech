...
for n_words in n_words_list:
    ...
    n_train.to_csv("experiments/temp_train.csv", index=False)
    n_val.to_csv("experiments/temp_val.csv", index=False)
    ...
    if (
        ...
        load_model_weights(
            model.base,
            Path(config.extractor.checkpoints)
            / f"base_best_{config.extractor.model}.pt",
            torch.device(device),
        )

    trainer = Trainer(
        "experiments/temp_train.csv",
        "experiments/temp_val.csv",
