    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        for round_num in range(n):
            ...
            if len(participant_videos) < k:
                print(f"Warning: Participant {participant_id} only has {len(participant_videos)} videos, using all of them")
                selected_videos = participant_videos
            else:
                selected_videos = participant_videos.sample(k)
            
            video_paths = [os.path.join(video_base_dir, row['Video file']) for _, row in selected_videos.iterrows()]
            combined_gloss = " ".join(selected_videos['Gloss'].tolist())
            
            # Concatenate videos
            concat_video_path = os.path.join(temp_dir, f"concat_video_round_{round_num}.mp4")
